...
//        kafka-topics.sh --create --topic doit30-events --partitions 3 --replication-factor 2 --bootstrap-server hadoop202:9092
        ...
        prop.load(Consumer01.class.getClassLoader().getSystemResourceAsStream("consumer.properties"));
...
// *       每 5分钟，输出一次截止到当时的数据中出现过的用户总数
            ...
            for (ConsumerRecord<String, String> record : records) {
                ...
                User user1 = JSONObject.parseObject(record.value(), User.class);
                Integer guid = user1.getGuid();
                if (System.currentTimeMillis()-startTime<60*1000){
                    ...
                    map.put(startTime,set);
                    System.out.println("********** five minutes leter ******************");
                    System.out.println(map.get(startTime).size());
